...
fn parse_node<'a>(env: &mut JNIEnv<'a>, object: &mut JavaASTObject<'a>) -> Option<Node> {
    ...
    while last_result == None {
        match ast_name.as_str() {
            ...
            "NISub" => {
                last_result = parse_nisub(env, object);
            },
            "NIDiv" => {
                last_result = parse_nidiv(env, object);
            },
            "NIMul" => {
                last_result = parse_nimul(env, object);
            },
            "NLAdd" => {
                last_result = parse_nladd(env, object);
            },
            "NLSub" => {
                last_result = parse_nlsub(env, object);
            },
            "NLMul" => {
                last_result = parse_nlmul(env, object);
            },
            "NLDiv" => {
                last_result = parse_nldiv(env, object);
            },
            "NLCmp" => {
                last_result = parse_nlcmp(env, object);
            },
            "NFAdd" => {
                last_result = parse_nfadd(env, object);
            },
            "NFSub" => {
                last_result = parse_nfsub(env, object);
            },
            "NFMul" => {
                last_result = parse_nfmul(env, object);
            },
            "NFDiv" => {
                last_result = parse_nfdiv(env, object);
            },
            "NDAdd" => {
                last_result = parse_ndadd(env, object);
            },
            "NDSub" => {
                last_result = parse_ndsub(env, object);
            },
            "NDMul" => {
                last_result = parse_ndmul(env, object);
            },
            "NDDiv" => {
                last_result = parse_nddiv(env, object);
            },
            "NArrayLength" => {
...
fn parse_nisub<'a>(env: &mut JNIEnv<'a>, object: &mut JavaASTObject<'a>) -> Option<Node> {
    if object.scan_stage == 0 {
        object.data = Node::NISub {
            left: Box::new(Node::Placeholder),
            right: Box::new(Node::Placeholder)
        };
        object.scan_stage += 1;
        ...
        let mut temp_data = object.data.clone();
        match temp_data {
            Node::NISub { ref mut left, ref mut right } => {
                *left = Box::new(left_value.unwrap().clone());
                *right = Box::new(right_value.unwrap().clone());
            },
            _ => panic!("NISub data isn't an NISub!")
        }
        object.data = temp_data;
        object.scan_stage += 1;
    } else {
...
fn parse_nidiv<'a>(env: &mut JNIEnv<'a>, object: &mut JavaASTObject<'a>) -> Option<Node> {
    if object.scan_stage == 0 {
        object.data = Node::NIDiv {
            left: Box::new(Node::Placeholder),
            right: Box::new(Node::Placeholder)
        };
        object.scan_stage += 1;
        ...
        let mut temp_data = object.data.clone();
        match temp_data {
            Node::NIDiv { ref mut left, ref mut right } => {
                *left = Box::new(left_value.unwrap().clone());
                *right = Box::new(right_value.unwrap().clone());
            },
            _ => panic!("NIDiv data isn't an NIDiv!")
        }
        object.data = temp_data;
        object.scan_stage += 1;
    } else {
...
fn parse_nladd<'a>(env: &mut JNIEnv<'a>, object: &mut JavaASTObject<'a>) -> Option<Node> {
    if object.scan_stage == 0 {
        object.data = Node::NLAdd {
            left: Box::new(Node::Placeholder),
            right: Box::new(Node::Placeholder)
        };
        object.scan_stage += 1;
        ...
        let mut temp_data = object.data.clone();
        match temp_data {
            Node::NLAdd { ref mut left, ref mut right } => {
                *left = Box::new(left_value.unwrap().clone());
                *right = Box::new(right_value.unwrap().clone());
            },
            _ => panic!("NLAdd data isn't an NLAdd!")
        }
        object.data = temp_data;
        object.scan_stage += 1;
    } else {
...
fn parse_nlsub<'a>(env: &mut JNIEnv<'a>, object: &mut JavaASTObject<'a>) -> Option<Node> {
    if object.scan_stage == 0 {
        object.data = Node::NLSub {
            left: Box::new(Node::Placeholder),
            right: Box::new(Node::Placeholder)
        };
        object.scan_stage += 1;
        ...
        let mut temp_data = object.data.clone();
        match temp_data {
            Node::NLSub { ref mut left, ref mut right } => {
                *left = Box::new(left_value.unwrap().clone());
                *right = Box::new(right_value.unwrap().clone());
            },
            _ => panic!("NLSub data isn't an NLSub!")
        }
        object.data = temp_data;
        object.scan_stage += 1;
    } else {
...
fn parse_nlmul<'a>(env: &mut JNIEnv<'a>, object: &mut JavaASTObject<'a>) -> Option<Node> {
    if object.scan_stage == 0 {
        object.data = Node::NLMul {
            left: Box::new(Node::Placeholder),
            right: Box::new(Node::Placeholder)
        };
        object.scan_stage += 1;
        ...
        let mut temp_data = object.data.clone();
        match temp_data {
            Node::NLMul { ref mut left, ref mut right } => {
                *left = Box::new(left_value.unwrap().clone());
                *right = Box::new(right_value.unwrap().clone());
            },
            _ => panic!("NLMul data isn't an NLMul!")
        }
        object.data = temp_data;
        object.scan_stage += 1;
    } else {
...
fn parse_nldiv<'a>(env: &mut JNIEnv<'a>, object: &mut JavaASTObject<'a>) -> Option<Node> {
    if object.scan_stage == 0 {
        object.data = Node::NLDiv {
            left: Box::new(Node::Placeholder),
            right: Box::new(Node::Placeholder)
        };
        object.scan_stage += 1;
        ...
        let mut temp_data = object.data.clone();
        match temp_data {
            Node::NLDiv { ref mut left, ref mut right } => {
                *left = Box::new(left_value.unwrap().clone());
                *right = Box::new(right_value.unwrap().clone());
            },
            _ => panic!("NLDiv data isn't an NLDiv!")
        }
        object.data = temp_data;
        object.scan_stage += 1;
    } else {
...
fn parse_nfadd<'a>(env: &mut JNIEnv<'a>, object: &mut JavaASTObject<'a>) -> Option<Node> {
    if object.scan_stage == 0 {
        object.data = Node::NFAdd {
            left: Box::new(Node::Placeholder),
            right: Box::new(Node::Placeholder)
        };
        object.scan_stage += 1;
        ...
        let mut temp_data = object.data.clone();
        match temp_data {
            Node::NFAdd { ref mut left, ref mut right } => {
                *left = Box::new(left_value.unwrap().clone());
                *right = Box::new(right_value.unwrap().clone());
            },
            _ => panic!("NFAdd data isn't an NFAdd!")
        }
        object.data = temp_data;
        object.scan_stage += 1;
    } else {
...
fn parse_nfsub<'a>(env: &mut JNIEnv<'a>, object: &mut JavaASTObject<'a>) -> Option<Node> {
    if object.scan_stage == 0 {
        object.data = Node::NFSub {
            left: Box::new(Node::Placeholder),
            right: Box::new(Node::Placeholder)
        };
        object.scan_stage += 1;
        ...
        let mut temp_data = object.data.clone();
        match temp_data {
            Node::NFSub { ref mut left, ref mut right } => {
                *left = Box::new(left_value.unwrap().clone());
                *right = Box::new(right_value.unwrap().clone());
            },
            _ => panic!("NFSub data isn't an NFSub!")
        }
        object.data = temp_data;
        object.scan_stage += 1;
    } else {
...
fn parse_nfmul<'a>(env: &mut JNIEnv<'a>, object: &mut JavaASTObject<'a>) -> Option<Node> {
    if object.scan_stage == 0 {
        object.data = Node::NFMul {
            left: Box::new(Node::Placeholder),
            right: Box::new(Node::Placeholder)
        };
        object.scan_stage += 1;
        ...
        let mut temp_data = object.data.clone();
        match temp_data {
            Node::NFMul { ref mut left, ref mut right } => {
                *left = Box::new(left_value.unwrap().clone());
                *right = Box::new(right_value.unwrap().clone());
            },
            _ => panic!("NFMul data isn't an NFMul!")
        }
        object.data = temp_data;
        object.scan_stage += 1;
    } else {
...
fn parse_nfdiv<'a>(env: &mut JNIEnv<'a>, object: &mut JavaASTObject<'a>) -> Option<Node> {
    if object.scan_stage == 0 {
        object.data = Node::NFDiv {
            left: Box::new(Node::Placeholder),
            right: Box::new(Node::Placeholder)
        };
        object.scan_stage += 1;
        ...
        let mut temp_data = object.data.clone();
        match temp_data {
            Node::NFDiv { ref mut left, ref mut right } => {
                *left = Box::new(left_value.unwrap().clone());
                *right = Box::new(right_value.unwrap().clone());
            },
            _ => panic!("NFDiv data isn't an NFDiv!")
        }
        object.data = temp_data;
        object.scan_stage += 1;
    } else {
...
fn parse_ndadd<'a>(env: &mut JNIEnv<'a>, object: &mut JavaASTObject<'a>) -> Option<Node> {
    if object.scan_stage == 0 {
        object.data = Node::NDAdd {
            left: Box::new(Node::Placeholder),
            right: Box::new(Node::Placeholder)
        };
        object.scan_stage += 1;
        ...
        let mut temp_data = object.data.clone();
        match temp_data {
            Node::NDAdd { ref mut left, ref mut right } => {
                *left = Box::new(left_value.unwrap().clone());
                *right = Box::new(right_value.unwrap().clone());
            },
            _ => panic!("NDAdd data isn't an NDAdd!")
        }
        object.data = temp_data;
        object.scan_stage += 1;
    } else {
...
fn parse_ndsub<'a>(env: &mut JNIEnv<'a>, object: &mut JavaASTObject<'a>) -> Option<Node> {
    if object.scan_stage == 0 {
        object.data = Node::NDSub {
            left: Box::new(Node::Placeholder),
            right: Box::new(Node::Placeholder)
        };
        object.scan_stage += 1;
        ...
        let mut temp_data = object.data.clone();
        match temp_data {
            Node::NDSub { ref mut left, ref mut right } => {
                *left = Box::new(left_value.unwrap().clone());
                *right = Box::new(right_value.unwrap().clone());
            },
            _ => panic!("NDSub data isn't an NDSub!")
        }
        object.data = temp_data;
        object.scan_stage += 1;
    } else {
...
fn parse_ndmul<'a>(env: &mut JNIEnv<'a>, object: &mut JavaASTObject<'a>) -> Option<Node> {
    if object.scan_stage == 0 {
        object.data = Node::NDMul {
            left: Box::new(Node::Placeholder),
            right: Box::new(Node::Placeholder)
        };
        object.scan_stage += 1;
        ...
        let mut temp_data = object.data.clone();
        match temp_data {
            Node::NDMul { ref mut left, ref mut right } => {
                *left = Box::new(left_value.unwrap().clone());
                *right = Box::new(right_value.unwrap().clone());
            },
            _ => panic!("NDMul data isn't an NDMul!")
        }
        object.data = temp_data;
        object.scan_stage += 1;
    } else {
...
fn parse_nddiv<'a>(env: &mut JNIEnv<'a>, object: &mut JavaASTObject<'a>) -> Option<Node> {
    if object.scan_stage == 0 {
        object.data = Node::NDDiv {
            left: Box::new(Node::Placeholder),
            right: Box::new(Node::Placeholder)
        };
        object.scan_stage += 1;
        ...
        let mut temp_data = object.data.clone();
        match temp_data {
            Node::NDDiv { ref mut left, ref mut right } => {
                *left = Box::new(left_value.unwrap().clone());
                *right = Box::new(right_value.unwrap().clone());
            },
